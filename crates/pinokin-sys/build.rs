//! Links the `par6_shim` C++ library. The shim is always linked: pixi
//! provides the C++ closure and `pixi run setup` builds it, so there is no
//! toolchain-free build to keep a feature gate for.
//!
//! Consumes:
