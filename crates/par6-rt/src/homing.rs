        ...
        match self.phase {
            HPhase::Approach => {
                    ...
                    && !self.preclear_used
                {
                    ...
                    self.phase = HPhase::Backoff { rehome_after: false };
                    self.elapsed = 0;
            ...
            HPhase::PostMove => {
                ...
                let cmd = JointCommand::position(
                    self.post_target_ticks,
                    trunc_to_wire(post.speed),
                    0,
                );
                let in_pos = node
                    .position_ticks
...
/// returns (position, velocity ticks/s) at `elapsed` of `dur` ticks.
fn hermite(start: f64, end: f64, elapsed: u32, dur: u32, dt: f64) -> (f64, f64) {
    ...
    let vel = d * (6.0 * s - 6.0 * s * s) / (f64::from(dur.max(1)) * dt);
    (pos, vel)
}
...
impl HomingSystem {
        ...
        let steps = robot
            ...
            .map(|s| StepPlan {
                ...
                home_joints: s
                    .home
                    .as_ref()
                    .map(|h| h.joints.iter().map(|j| usize::from(*j)).collect())
                    .unwrap_or_default(),
        ...
        *gcmd = if self.has_can_gripper {
        ...
        match self.part {
            ...
            Part::HomeStart => {
                ...
                let joints = self.steps[self.step_idx].home_joints.clone();
                for &j in &joints {
                    let p = &self.params[j];
                    let _ = bus.send_limits(
                        p.node,
    ...
    fn tick_home<B: DriverBus>(
        ...
        for &j in &joints {
            ...
            match event {
                Some(HomerEvent::Reference { latched_ticks }) => {
                    ...
                    if p.post.is_some() {
                        self.homers[j].post_target_ticks = conv[j].motor_ticks(
                            p.post.as_ref().expect("post plan").position_rad,
                        );
                    }
                }
        ...
        let arm_done = joints.iter().all(|&j| !self.homers[j].running());
