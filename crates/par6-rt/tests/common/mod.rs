...
/// The gravity torque the simulated plant actually applies at `q` \[Nm\].
///
/// These torques used to be pinned constants regenerated by hand from an
/// example binary. `bundle()` reads the shipped config, so flipping
/// `active_tool` -- the documented way to record a tool change -- staled
/// them silently and the joint just sagged by whatever the model no longer
/// accounted for. Asking the plant makes a tool change something the test
/// follows rather than something that breaks it, and makes the feedforward
/// exact for whatever tool is configured.
///
/// That the plant's gravity and the controller's Pinocchio model agree is a
/// separate claim, tested by par6d's `dynamics_conformance` suite; this is
/// only about the landing.
pub fn plant_gravity(bundle: &ConfigBundle, q: &[f64; MAX_JOINTS]) -> [f64; MAX_JOINTS] {
    ...
    let tau = bus
        .gravity_at(&q[..n])
        .expect("the sim scene carries a torque-level plant");
    std::array::from_fn(|i| tau.get(i).copied().unwrap_or(0.0))
}
