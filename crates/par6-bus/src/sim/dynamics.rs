...
impl DynamicsPlant {
    ...
    pub fn new(urdf: &Path, maps: &[JointMap], q0: &[f64]) -> Self {
        ...
        let k_stop: Vec<f64> = inertia.iter().map(|i| i * STOP_OMEGA * STOP_OMEGA).collect();
        ...
        for _ in 0..SUBSTEPS {
            for j in 0..self.q.len() {
                ...
                if self.q[j] > map.hard_hi_rad {
                    t += -self.k_stop[j] * (self.q[j] - map.hard_hi_rad) - self.c_stop[j] * self.v[j];
                } else if self.q[j] < map.hard_lo_rad {
                    t += -self.k_stop[j] * (self.q[j] - map.hard_lo_rad) - self.c_stop[j] * self.v[j];
                }
                self.tau[j] = t;
            }
            self.model
