...
impl GripperSim {
    pub fn new(dt: f64, node: NodeId, cfg: &GripperConfig) -> Self {
        ...
        let ticks_per_meter =
            16384.0 / (4.0 * std::f64::consts::PI * d.gear_r_m);
        ...
        Self {
            ...
            joint: KinJoint::new(dt, stroke_ticks / 2.0, 0.0, stroke_ticks, accel_max, d.ilim_ma),
            stroke_ticks,
            ctrl: Ctrl::Motor,
            pos_byte: 127.5,
    ...
    pub fn step(&mut self, dt: f64) {
        match self.ctrl {
            Ctrl::Motor => {
                ...
                self.pos_byte = 255.0 * (1.0 - (self.joint.pos / self.stroke_ticks).clamp(0.0, 1.0));
            }
            Ctrl::Firmware => {
    ...
    fn firmware_step(&mut self, dt: f64) {
        if self.calibration_ticks_left > 0 {
            ...
            let progress = 1.0
                - self.calibration_ticks_left as f64 / self.calibration_total_ticks as f64;
        ...
        self.moving = false;
        if self.driver.watchdog_fired() || self.estop_latched || !self.cmd.activate || !self.cmd.action
        {
            self.pressing = false;
            return;
        }
