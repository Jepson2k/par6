#!/usr/bin/env bash
# Build the C++ FFI stack for ANOTHER architecture, so the control box can be
# handed a runtime it did not compile. A native shim is not built here:
# `crates/par6-kin/build.rs` compiles it into cargo's OUT_DIR under the pixi
# environment, which is what `pixi run setup` does.
#
...
#   2. downloads (never executes) a target-platform conda-forge env with
#      Pinocchio, coal, urdfdom, eigen and libmujoco, and a host-platform env
#      with the target's cross compiler
#   3. builds + installs toppra-cpp (pinned commit; no conda-forge package)
#      from source into the target env prefix
#   4. builds + installs cpp/ (the par6_shim C-ABI library) against both
#   5. stages the runtime closure with `stage_runtime_libs.py`, which proves
#      it loadable on the target's glibc — the only check available without
#      target hardware
#   6. persists the env vars par6-kin's build.rs and the deploy scripts read
#
...
if [[ "$TARGET_ARCH" == "$HOST_ARCH" ]]; then
  echo "setup.sh builds for another architecture only; a native shim is built" >&2
  echo "by crates/par6-kin/build.rs. Use \`pixi run setup\`." >&2
  exit 2
fi
...
MUJOCO_VERSION="${PAR6_MUJOCO_VERSION:-3.12.0}"
...
# Build tools live in a host-platform env holding the target's cross compiler.
# The shim is compiled by the cross gcc but loads the target env's
# libstdc++ at runtime, so that libstdc++ must be at least as new as the
# compiler's. Naming the runtimes here lets the solver take the current one
# instead of whatever pinocchio's floor happens to be.
...
# `cmake`/`ninja` and the compiler come from the HOST-platform toolchain env.
run_tool() { "$MAMBA" run -p "$TOOLCHAIN_DIR" "$@"; }
cmake_cross_args=(-DCMAKE_TOOLCHAIN_FILE="$TOOLCHAIN_FILE")
# $ORIGIN, because the staged closure and the shim sit in one directory both
# here and at /usr/local/lib/par6 on the box.
DEP_RPATH='$ORIGIN'
...
JOBS
  echo "# libmujoco lives in the env prefix. The name is mujoco-rs's own:"
  echo "# its build script links what this points at instead of downloading"
  echo "# a copy, so the simulator uses the same closure everything else does."
  echo "export MUJOCO_DYNAMIC_LINK_DIR=\"$ENV_DIR/lib\""
