...
- **Pinocchio** (kinematics/dynamics) — `par6_kin_*` (create/destroy, fk,
  jacobian, gravity, DLS ik_step), consumed by `crates/pinokin-sys`
...
| Env var | Meaning |
|---|---|
| `PAR6_SHIM_LIB_DIR` | required; dir containing `libpar6_shim.{so,a}` (the pixi activation exports it) |
...
The crate always links the shim: it is a member of the root workspace, and
every `cargo` invocation runs under `pixi run`, whose activation exports
these.
...
`crates/pinokin-sys/tests/fixtures/par6_flange_pin.json`. The Rust tests
load the same URDF through the shim and must match to **1e-9
...
pixi's manifest lists `linux-aarch64` alongside `linux-64`, and conda-forge
ships the whole C++ closure for it, so the control box builds natively:
`pixi run setup` there produces its own shim. CI cross-*checks* every crate
for aarch64 from the x86_64 runner (a check never links, so the host shim
satisfies `build.rs`); the deploy bundle's cross-compiled shim still comes
from `scripts/ffi/setup.sh --target aarch64`.
