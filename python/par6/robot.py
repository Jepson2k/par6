...
class Robot(_RobotABC):
    ...
    def start(self, **kwargs: Any) -> None:
        """Connect to a running par6d, or spawn ``par6d --sim`` with ``sim=True``.

        Keyword args override constructor defaults: ``host``, ``port``,
        ``timeout``, plus ``sim`` (bool).  Blocks until the runtime answers
        PING; raises ``RuntimeError`` on failure.
        """
        host: str = kwargs.get("host", self._host)
        port: int = kwargs.get("port", self._port)
        timeout: float = kwargs.get("timeout", self._timeout)
        sim: bool = bool(kwargs.get("sim", False))

        if not sim:
            if not _ping_runtime(host, port, timeout=min(timeout, 2.0)):
                raise RuntimeError(f"par6d runtime not reachable at {host}:{port}")
            return

        if _ping_runtime(host, port):
            raise RuntimeError(f"a runtime is already running at {host}:{port}")
